...
if(!user.equals("supervisor") && session.getAttribute("id_class")!=null){
    ...
    /*      READ EXCEL FILE IN FORM - USERNAME,NAME,LASTNAME   
            AND REMOVE THE STUDENT FROM THE CLASS
            ...
            try {
                ...
                while (iter.hasNext()) {
                    ...
                    if (!item.isFormField()) {
                            ...
                            for (Row row: sheet) {
                                if(row.getLastCellNum()==4 && row.getRowNum()>0){
                                    ...
                                    if(id_userExcel!=null){
                                        ...
                                        if(alreadyInExcel>0){
                                            //remove student
                                            sql = "delete from class_has_user where id_user = '"+id_userExcel+"' and"
                                                    + " id_class = '"+id_classExcel+"'";
                        ...
                        connection.close();
                        out.print("<script>window.location.replace('supervisor-class-remove.jsp');</script>");
                    }
                }
